...
// Navigation tab definitions extracted to prevent recreation on every render
...
export default function App() {
  ...
  const leaderboardUsers = useMemo(() => {
    ...
    if (userEmissions !== null) {
      const alreadyExists = list.some((item) => item.isUser || (user && item.user_id === user.id));
      if (!alreadyExists) {
        list.push({
  ...
  // Navigation tab definitions are declared statically above the component
