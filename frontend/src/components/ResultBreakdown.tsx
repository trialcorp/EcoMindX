...
export function ResultBreakdown({ result, input }: Props) {
  ...
  return (
    <section className="card" aria-labelledby="result-heading">
      ...
      <div className="dashboard-metrics-grid">
        ...
        <div className="metric-card">
          ...
          <span className="value">
            {result.comparison.ratio_to_global_average.toFixed(1)}×
          </span>
          <span className="sub">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem", alignItems: "center", marginBottom: "2rem" }} className="gauge-responsive-split">
        <div className="gauge-wrapper">
          ...
          <div className="gauge-text">
            <span style={{ fontSize: "1.5rem", fontWeight: 700, color: "#fff" }}>{sustainabilityScore}%</span>
            <span style={{ fontSize: "0.7rem", color: "var(--muted)", textTransform: "uppercase", fontWeight: 600 }}>Sustain Index</span>
          </div>
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          <p style={{ margin: 0, fontSize: "1.05rem", fontWeight: 500 }}>
            Your sustainability index is <strong className={overTarget ? "over" : "under"}>{sustainabilityScore}%</strong>.
          </p>
          <p style={{ margin: 0, color: "var(--muted)", fontSize: "0.9rem" }}>
            This footprint represents <strong>{result.comparison.ratio_to_sustainable_target.toFixed(1)}×</strong> the sustainable climate target and <strong>{result.comparison.ratio_to_global_average.toFixed(1)}×</strong> the global individual average emissions.
          </p>
        </div>
      </div>
      ...
      {input && (
        <div className="card category-detail-card" style={{ marginTop: "1.5rem", background: "rgba(255, 255, 255, 0.02)", border: "1px dashed rgba(16, 185, 129, 0.25)" }}>
          <h4 style={{ margin: "0 0 1rem 0", color: "var(--primary-hover)", display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "1.05rem", fontFamily: "var(--font-display)" }}>
          ...
          <div className="detail-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem" }}>
            {selectedCategory === "transport" && (
              <>
                <div className="detail-item">
                  ...
                  <span className="detail-desc">Fuel type: <strong style={{ textTransform: "capitalize" }}>{input.transport.car_fuel}</strong></span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Public Transit</span>
                  <span className="detail-value">{input.transport.public_transit_km_per_week} km/week</span>
                  <span className="detail-desc">Trains, buses, and subways</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Aviation (Flights)</span>
                  <span className="detail-value">
                    {input.transport.short_haul_flights_per_year + input.transport.long_haul_flights_per_year} flights/year
                  </span>
                  <span className="detail-desc">
                    Short-haul: {input.transport.short_haul_flights_per_year} • Long-haul: {input.transport.long_haul_flights_per_year}
                  </span>
                </div>
              </>
            )}
            ...
            {selectedCategory === "home" && (
              <>
                <div className="detail-item">
                  ...
                  <span className="detail-value">{input.home.electricity_kwh_per_month} kWh/month</span>
                  <span className="detail-desc">Household electricity consumption</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Natural Gas Usage</span>
                  <span className="detail-value">{input.home.natural_gas_kwh_per_month} kWh/month</span>
            ...
            {selectedCategory === "diet" && (
              <>
                <div className="detail-item">
                  ...
                  <span className="detail-desc">
                    {input.diet === "heavy_meat" && "High impact due to carbon-heavy beef and lamb."}
                    {input.diet === "medium_meat" && "Typical mixed diet with moderate meat consumption."}
                    {input.diet === "low_meat" && "Reduced meat intake; lower environmental footprint."}
                    {input.diet === "pescatarian" && "Fish and plants only; no red meat or poultry."}
                    {input.diet === "vegetarian" && "No meat or fish; includes dairy and eggs."}
                    {input.diet === "vegan" && "Purely plant-based; lowest possible carbon emissions."}
                  </span>
                </div>
              </>
            )}
            ...
            {selectedCategory === "consumption" && (
              <>
                <div className="detail-item">
                  ...
                  <span className="detail-value">${input.consumption.goods_spend_usd_per_month}/month</span>
                  <span className="detail-desc">Purchases of clothes, electronics, furniture, etc.</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Waste Generated</span>
                  <span className="detail-value">{input.consumption.waste_kg_per_week} kg/week</span>
      ...
      <table className="history">
        ...
        <tbody>
          {entries.map(([key, value]) => (
            <tr key={key}>
              <th scope="row" style={{ color: "var(--muted)" }}>{categoryLabel(key)}</th>
