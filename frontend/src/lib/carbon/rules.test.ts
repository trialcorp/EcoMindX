...
describe("Rules Engine", () => {
  ...
  const baseResult: FootprintResult = {
    ...
    comparison: { global_average_annual_kg: 4800, sustainable_target_annual_kg: 2000, ratio_to_global_average: 0.2, ratio_to_sustainable_target: 0.5 },
  };
  ...
  it("suggests electric vehicles if driving petrol/diesel", () => {
    const transportResult = { ...baseResult, breakdown_kg: { ...baseResult.breakdown_kg, transport: 2000 } };
    const input: CarbonInput = { ...baseInput, transport: { ...baseInput.transport, car_km_per_week: 200, car_fuel: "petrol" } };
  ...
  it("suggests flying less if flights dominate transport emissions", () => {
    const transportResult = { ...baseResult, breakdown_kg: { ...baseResult.breakdown_kg, transport: 5000 } };
    const input: CarbonInput = { ...baseInput, transport: { ...baseInput.transport, long_haul_flights_per_year: 3 } };
    const insights = generateRuleBasedInsights(input, transportResult);
