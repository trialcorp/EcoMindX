...
function _transport_annual_kg(t: CarbonInput["transport"]): number {
  ...
  const transit = t.public_transit_km_per_week * factors.WEEKS_PER_YEAR * factors.PUBLIC_TRANSIT_PER_KM;
  const flights =
...
function _home_annual_kg(h: CarbonInput["home"]): number {
  const electricity = h.electricity_kwh_per_month * factors.MONTHS_PER_YEAR * factors.ELECTRICITY_PER_KWH;
...
function _consumption_annual_kg(c: CarbonInput["consumption"]): number {
  const goods = c.goods_spend_usd_per_month * factors.MONTHS_PER_YEAR * factors.GOODS_PER_USD_MONTHLY;
...
export function calculateFootprint(data: CarbonInput): FootprintResult {
  ...
  const total = round(breakdown.transport + breakdown.home + breakdown.diet + breakdown.consumption, 2);
